...
int main( int argc, char *argv[] )
{
  ...
  FixedImageMaskType::Pointer movingSpatialObjectMask = FixedImageMaskType::New();
  FixedImageMaskType::Pointer fixedSpatialObjectMask = FixedImageMaskType::New();
  ...
  movingMaskImageReader->SetFileName( argv[3] );
  fixedMaskImageReader->SetFileName( argv[4] );
  ...
  for (int imageIndex=1; imageIndex<numOfImages; imageIndex++) {
	  ...
	  movingImageReader->SetFileName(  argv[7] );
	  fixedImageReader->SetFileName( argv[7+imageIndex] );

	  std::string movingImageName(argv[7]);
	  std::string slash = "/";
	  std::size_t movingSlashIndex = movingImageName.find_last_of(slash);
	  std::string fixedImageName(argv[7+imageIndex]);
	  ...
	  movingSpatialObjectMask->SetImage( movingMaskImage );
	  metric->SetMovingImageMask(movingSpatialObjectMask);
	  fixedSpatialObjectMask->SetImage( fixedMaskImage );
	  ...
	  it.GoToBegin();
	  while( !it.IsAtEnd() ) {
		if(movingMaskImage->GetPixel(it.GetIndex()) == 0) {
			it.Set(zeroVector);
		}
	    ++it;
	  }
