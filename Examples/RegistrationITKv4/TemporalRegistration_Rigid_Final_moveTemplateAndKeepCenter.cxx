...
int main( int argc, char *argv[] )
{
  ...
  for (int imageIndex=1; imageIndex<numOfImages; imageIndex++) {
	  ...
	  spatialObjectFixedMask->SetImage( movingMaskImage );
	  metric->SetMovingImageMask(spatialObjectFixedMask);

	  maskFilter->SetInput( movingImage );
	  maskFilter->SetMaskImage( movingMaskImage );
	  MovingImageType::ConstPointer maskedMovingImage = maskFilter->GetOutput();
	  maskFilter->Update();

	  typedef itk::ImageMomentsCalculator< FixedImageType > MovingImageCalculatorType;
	  MovingImageCalculatorType::Pointer MovingImageCalculator = MovingImageCalculatorType::New();
	  MovingImageCalculator->SetImage( maskedMovingImage );
